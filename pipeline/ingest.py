...
def to_3m(df: pd.DataFrame) -> pd.DataFrame:
    ...
    midnight origin lands bins on 08:30, 08:33, 08:36 … — the same grid the
    original app uses. Do not change the origin.
    """
    if df.index.tz is None:
        df.index = df.index.tz_localize("UTC")
    df = df.tz_convert(SESSION_TZ)
 
    out = df.resample(f"{BAR_MINUTES}min", label="left", closed="left").agg(
...
def main() -> int:
    ...
    # inputs are empty) yields [] rather than the default — which would make
    # the job exit green having ingested nothing. Fall back explicitly.
    if not args.symbols:
